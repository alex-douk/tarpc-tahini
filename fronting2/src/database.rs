...
pub(crate) async fn register_user(username: BBox<String, UsernamePolicy>) -> BBox<String, UsernamePolicy>{
    let codec_builder = LengthDelimitedCodec::builder();
    let stream = TcpStream::connect((SERVER_ADDRESS, 5002)).await.unwrap();
    let transport = new_transport(codec_builder.new_framed(stream), Json::default());
    let response = TahiniDatabaseClient::new(Default::default(), transport)
        .spawn()
        .register_user(tarpc::context::Context::current(), username).await;
