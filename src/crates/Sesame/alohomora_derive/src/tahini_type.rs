...
fn handle_enum(attrs: AlohomoraTypeArgs, input: ItemEnum) -> Result<TokenStream, Error> {
    ...
    let var_strings : Vec<_> = input.variants.iter().map(|v| v.ident.clone().to_string()).collect();
...
fn handle_unnamed_variant(var: &Variant) -> Result<TokenStream, Error> {
    match var.fields.len() {
        1 => Ok(quote! { ::alohomora::tarpc::enums::TahiniVariantsEnum::NewType(Box::new(x.to_tahini_enum()))}),
        _ => {
            Err((
                var.ident.span(),
