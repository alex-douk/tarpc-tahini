...
declare_alohomora_lint!(
    ...
    pub ALOHOMORA_SANDBOX_TRANSFER,
    Deny, // does not allow override
    "FastTransfer must always be auto-generated by the FastTransfer macro, not user-implemented",
...
fn contains_secret(cx: &LateContext<'_>, def_id: &DefId) -> bool {
    let secret = "Library implementation of FastTransfer. Do not copy this docstring!";
...
fn error_message(cx: &LateContext<'_>, &def_id: &DefId) {
    ...
    match map.span_if_local(def_id.clone()) {
        None => {
            panic!("Manual implementation of FastTransfer trait at {}. doc: {:?}",
        ...
        Some(span) => {
            span_lint_and_help (
                cx,
                ALOHOMORA_SANDBOX_TRANSFER,
                span,
                "\x1b[93mmanual implementation of FastTransfer trait\x1b[0m",
                None, "use `#[derive(FastTransfer)]` instead"
            );
        }
    }
}
...
fn check_crate(cx: &LateContext<'_>) {
    let path: &[&str] = &vec!["alohomora_sandbox", "FastTransfer"];
