...
use core_tahini_utils::{
    funcs::marketing_parse_conv, types::{Message}
};
use futures::executor::block_on;
use advertisement_tahini_utils::{service::AdvertisementClient, types::MarketingData};
...
async fn initialize_ad_client() -> AdvertisementClient {
    let codec_builder = LengthDelimitedCodec::builder();
    let stream = TcpStream::connect((SERVER_ADDRESS, 8002)).await.unwrap();
    let transport = new_transport(codec_builder.new_framed(stream), Json::default());

    let client = AdvertisementClient::new(Default::default(), transport)
        .spawn();
    client
}
...
pub async fn benchmark_ads(nb_iter: usize, rounds: usize) {
    ...
    let client = initialize_ad_client().await;
    for i in 0..nb_iter {
        let _ = contact_ad_server(&client, data.clone(), i).await;
    }
