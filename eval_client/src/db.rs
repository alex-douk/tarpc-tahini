...
async fn login(
    ...
    uuid.expect("User already exists")
}
...
async fn retrieve_conversation(client: &DatabaseClient, uuid: String, conv_id: String, iter: usize) {
    let context = tarpc::context::current();
    let start = Instant::now();
...
pub async fn benchmark_db(nb_iters: usize, rounds: usize) {
    ...
    let (client, uuid) = block_on(async {
        ...
        let uuid = login(&c, username).await;
        (c, uuid)
    });
    ...
    for _ in 0..(nb_iters/ROUNDS) {
        ...
        for message in conversation {
            conv_id = Some(store_to_database(&client, uuid.clone(), conv_id, message).await);
        }
        conv_ids.insert(conv_id.expect(""));
    }
    println!("DB_Store done");

    let conv_ids = conv_ids.drain();
    for (i, cid) in conv_ids.enumerate(){
        retrieve_conversation(&client, uuid.clone(), cid, i).await;
    }
