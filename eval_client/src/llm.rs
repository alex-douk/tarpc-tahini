...
async fn initialize_llm_client() -> InferenceClient {
    ...
    let client = InferenceClient::new(Default::default(), transport)
        .spawn();
    client
}
...
pub async fn benchmark_llm(nb_iter: usize, rounds: usize) {
    let prompt = UserPrompt {
        conversation: gen_conversation(rounds),
        nb_token: 250,
    };
    let client = initialize_llm_client().await;
    for i in 0..nb_iter {
